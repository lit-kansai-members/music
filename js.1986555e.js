...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"hIs6":[function(require,module,exports) {
var $mark = document.getElementById("mark");

// move mark to center of viewport.
$mark.style.transform = "translateX(".concat(window.innerWidth / 2 - document.querySelector("#mark").getBoundingClientRect().left, "px) translateX(-50%)");
$mark.style.transition = "1s";
var paths = Array.from(document.querySelectorAll("path.first, path.second"));
...
},{}],"m9uI":[function(require,module,exports) {
var words = ["なんと！　ななんと！　なななんと！", "Life is Tech ! は気が利くので〜", "さっすが～ 知らなかった～ すっご～い センスあるね そ～なんだ～", "おおっ！？", "I'm a 注意点 Human.", "探さない、待つの。", "勝手に水分摂らないで", "①かわく ②疲れる ③おひるま眠る ④深夜に目覚める ⑤いなくなる"];
...
document.querySelectorAll(".close-modal").forEach(function (v) {
  return v.addEventListener("click", function (e) {
    ...
    e.preventDefault();
    document.querySelectorAll(".modal.open").forEach(function (v) {
      return v.classList.remove("open");
    });
  });
});
document.querySelectorAll(".open-modal").forEach(function (v) {
  return v.addEventListener("click", function (e) {
    document.body.style.overflow = "hidden";
    e.preventDefault();
    document.getElementById(v.getAttribute("href").slice(1)).classList.add("open");
  });
});
var $modals = document.querySelectorAll(".modal");
window.addEventListener("keydown", function (_ref) {
  var key = _ref.key;
  return key === "Escape" && $modals.forEach(function (modal) {
    return modal.classList.remove("open");
  });
});
},{}],"bekQ":[function(require,module,exports) {
/**
...
var css = "";
Array.from(document.querySelectorAll("#navigations .year")).forEach(function (el, i) {
  var selector = "#navigations > .year:nth-child(".concat(i + 1, ")");
  var inneryear = el.querySelector(".inneryear");
  var outerCamp = el.querySelector(".outerCamp");
  css += "\n".concat(selector, ".opened, ").concat(selector, ":hover {\n  --width: ").concat(inneryear.offsetWidth + 2, "px;\n}");
  if (outerCamp) {
    var lastCamp = outerCamp.querySelector(".camp:last-child");
    css += "\n".concat(selector, ".full-opened {\n  --width: ").concat(outerCamp.offsetWidth + 2, "px;\n}\n").concat(selector, ".full-opened .outerCamp {\n  height: ").concat(lastCamp.offsetTop + lastCamp.offsetHeight, "px;\n}\n");
  }
});
style.innerHTML = css;
...
Array.from(document.querySelectorAll(".inneryear, .outerCamp")).forEach(function (v) {
  var year = (v.classList.contains("inneryear") ? v.parentElement : v).parentElement;
  v.addEventListener("mouseenter", function (e) {
    return year.classList.add("full-opened");
  });
  v.addEventListener("mouseleave", function (e) {
    return year.classList.remove("full-opened");
  });
});
...
Array.from(document.getElementsByClassName("outerCamp")).forEach(function (outerCamp) {
  return outerCamp.addEventListener("transitionend", function (e) {
    var year = outerCamp.parentElement;
...
var years = Array.from(document.querySelectorAll("header, main > h2 ,footer")).reverse();
var yearNavs = Array.from(document.querySelectorAll("#navigations > .year")).reverse();
var opened = 0;
window.addEventListener("scroll", function (e) {
  var VHHalf = window.innerHeight / 2;
  var open = years.findIndex(function (e) {
    return e.getBoundingClientRect().top < VHHalf;
  });
...
},{}],"J4PH":[function(require,module,exports) {
var define;
...
},{}],"xWRB":[function(require,module,exports) {
"use strict";
...
window.onYouTubeIframeAPIReady = function () {
  var playing;
  var isMobile = /.+(ipad|iphone|ipod|android|phone).+/.test(navigator.userAgent.toLowerCase());
  var $toggleButton = document.querySelector(".controller .toggle");
  var $backButton = document.querySelector(".controller .back");
  var $nextButton = document.querySelector(".controller .next");
  var $title = document.querySelector(".playing .title");
  var $author = document.querySelector(".playing .author");
  var $thumbnail = document.querySelector(".playing .thumb");
  var $player = document.querySelector(".player");
  var $toggleSetting = document.querySelector(".toggle-setting");
...
var _sweetScroll = _interopRequireDefault(require("sweet-scroll"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
var afterScroll = function afterScroll(toScroll, trigger) {
  return location.hash = trigger.hash.slice(1);
};
var scroll = new _sweetScroll.default({
  trigger: "[data-scroll]:not(.bounce)",
  afterScroll: afterScroll
});
var scrollBounce = new _sweetScroll.default({
